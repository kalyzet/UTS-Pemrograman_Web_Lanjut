...
@section('content')
	<body class="font-[Poppins] pb-[72px]">
		...
		<section id="Featured" class="mt-[30px]">
			<div class="w-full main-carousel"> @forelse ($featured_articles as $article) <div class="featured-news-card relative w-full h-[550px] flex shrink-0 overflow-hidden"> <img src="{{ Storage::url($article->thumbnail) }}" class="absolute object-cover w-full h-full thumbnail" alt="icon" /> <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div> <div class="card-detail max-w-[1130px] w-full mx-auto flex items-end justify-between pb-10 relative z-20"> <div class="flex flex-col gap-[10px]"> <p class="text-white">Terpopuler</p> <a href="{{ route('front.details', $article->slug) }}" class="font-bold text-4xl leading-[45px] text-white two-lines hover:underline transition-all duration-300"> {{ $article->name }} </a> <p class="text-white"> {{ $article->created_at->translatedFormat('d F Y') }} • {{ $article->category->name }} </p> </div> <div class="prevNextButtons flex items-center gap-4 mb-[60px]"> <button class="button--previous appearance-none w-[38px] h-[38px] flex items-center justify-center rounded-full shrink-0 ring-1 ring-white hover:ring-2 hover:ring-[#1865ff] transition-all duration-300"> <img src="assets/images/icons/arrow.svg" alt="arrow" /> </button> <button class="button--next appearance-none w-[38px] h-[38px] flex items-center justify-center rounded-full shrink-0 ring-1 ring-white hover:ring-2 hover:ring-[#1865ff] transition-all duration-300 rotate-180"> <img src="assets/images/icons/arrow.svg" alt="arrow" /> </button> </div> </div> </div> @empty <p>belum ada data terbaru...</p> @endforelse </div>
    ...
    @forelse ($articles as $article)
        <a href="{{ route('front.details', $article->slug) }}" class="h-full card-news">
            <div class="rounded-[20px] ring-1 ring-[#EEF0F7] p-[26px_20px] flex flex-col gap-4 hover:ring-2 hover:ring-[#1865ff] transition-all duration-300 bg-white h-full">
                <div class="thumbnail-container w-full h-[200px] rounded-[20px] flex shrink-0 overflow-hidden relative">
                    <p class="badge-white absolute top-5 left-5 rounded-full p-[8px_18px] bg-white font-bold text-xs leading-[18px] uppercase">{{ $article->category->name }}</p>
    ...
    @forelse ($authors as $author)
        <a href="{{ route('front.author', $author->slug) }}" class="h-full card-authors">
...
<a href="{{ route('front.category', 'hiburan') }}"
...
Jelajahi
				...
				<div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
					...
					<div class="card-detail w-full flex items-end p-[30px] relative z-20">
						<div class="flex flex-col gap-[10px]">
							...
							<p class="text-white">
                                {{ $entertainment_featured_articles->created_at->translatedFormat('d F Y') }}
                            </p>
						</div>
					</div>
				</div>
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('front.category', 'bisnis') }}" class="rounded-full p-[12px_22px] flex gap-[10px] font-semibold transition-all duration-300 border border-[#EEF0F7] hover:ring-2 hover:ring-[#189bff]">Jelajahi</a>
    </div>
    <div class="flex items-center justify-between h-fit">
        <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
            <img src="{{ Storage::url($business_featured_articles->thumbnail) }}" class="absolute object-cover w-full h-full thumbnail" alt="icon" />
            <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
            <div class="card-detail w-full flex items-end p-[30px] relative z-20">
                <div class="flex flex-col gap-[10px]">
                    <p class="text-white">Terpopuler</p>
                    <a href="{{ route('front.details', $business_featured_articles->slug) }}" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">
                        {{ $business_featured_articles->name }}
                    </a>
                    <p class="text-white">
                        {{ $business_featured_articles->created_at->translatedFormat('d F Y') }}
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('front.category', 'olahraga') }}"
   ...
   Jelajahi
    ...
    <div class="flex items-center justify-between h-fit">
        {{-- Featured Sport --}}
        @if ($sport_featured_articles)
        <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
            <img src="{{ Storage::url($sport_featured_articles->thumbnail) }}"
            ...
            <div class="card-detail w-full flex items-end p-[30px] relative z-20">
                <div class="flex flex-col gap-[10px]">
                    <p class="text-white">Terpopuler</p>
                    <a href="{{ route('front.details', $sport_featured_articles->slug) }}"
                       class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">
                        {{ $sport_featured_articles->name }}
                    </a>
                    <p class="text-white">
                        {{ $sport_featured_articles->created_at->translatedFormat('d F Y') }}
                    </p>
                </div>
            </div>
        </div>
