...
<body class="font-[Poppins]">
	...
	<header class="flex flex-col items-center gap-[50px] mt-[70px]">
		<div id="Headline" class="max-w-[1130px] mx-auto flex flex-col gap-4 items-center">
			<p class="w-fit text-[#A3A6AE]">{{ $articleNews->created_at->translatedFormat('d F Y') }} • {{ $articleNews->category->name }}</p>
    ...
    <p class="font-medium text-sm leading-[21px] text-[#A3A6AE] flex gap-1">
        Iklan Kami <a href="#" class="w-[18px] h-[18px]"><img
                src="{{ asset('assets/images/icons/message-question.svg') }}" alt="icon" /></a>
    </p>
@else
    <div class="text-sm italic text-gray-400">No ads available</div>
@endif

			</div>
			<div id="More-from-author" class="flex flex-col gap-4">
				<p class="font-bold">Lainnya Dari Penulis</p>
                @forelse ($author_news as $item_news)
                    <a href="{{ route('front.details', $item_news->slug) }}" class="card-from-author">
                        <div
                            class="rounded-[20px] ring-1 ring-[#EEF0F7] p-[14px] flex gap-4 hover:ring-2 hover:ring-[#18a6ff] transition-all duration-300">
                            ...
                            <div class="flex flex-col gap-[6px]">
                                ...
                                <p class="text-xs leading-[18px] text-[#A3A6AE]"> {{ $item_news->created_at->translatedFormat('d F Y') }} • {{ $item_news->category->name }}</p>
                            </div>
                        </div>
                    </a>
                @empty
		...
		<div class="max-w-[1130px] mx-auto flex flex-col gap-[30px]">
			...
			<div class="grid grid-cols-3 gap-[30px]">
                ...
                <a href="{{ route('front.details', $article->slug) }}" class="card">
                    <div
                        class="flex flex-col gap-4 p-[26px_20px] transition-all duration-300 ring-1 ring-[#EEF0F7] hover:ring-2 hover:ring-[#18a6ff] rounded-[20px] overflow-hidden bg-white">
